...
#def profile_page():
    if 'username' in session:
        ...
        # Retrieve water intake data for the logged-in user from the database
        conn = sqlite3.connect('fitness_tracker.db')
        c = conn.cursor()
        c.execute("SELECT * FROM water_intake WHERE user_id=?", (user_id,))
        water_intake_data = c.fetchall()
        conn.close()
        return render_template('profile.html', water_intake_data=water_intake_data)
    else:
        return redirect(url_for('home'))
#@app.route('/profile')
#def profile_page():
    if 'user_id' in session:
        ...
        water_intake_data = cursor.fetchall()
        # Close database connection
        conn.close()
        return render_template('profile.html', water_intake=water_intake_data)
    else:
